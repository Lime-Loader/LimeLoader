...
**LimeLoader** is an Android/IL2CPP Unity mod loader based on **LemonLoader 0.6.x**, updated
with **MelonLoader 0.7.x** fixes so it works with the **newest Unity versions** (Unity 6 /
`6000.x`). It also names its internal setup GameObject randomly on every run.

See [`LIMELOADER.md`](LIMELOADER.md) for the full list of changes, build status, and build
instructions. The original LemonLoader/MelonLoader credits and license notices below are
retained.
